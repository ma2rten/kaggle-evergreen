...
def get_scores(data):
    ...
    for train_idx, test_idx in KFold(len(labels), 10):
        scores.append(predict(data[train_idx], labels[train_idx], data[test_idx]))

    scores.append(predict(data[:len(labels)], labels, data[len(labels):]))
...
def get_lda(name, n):
    data = get_featureset(name, tf_idf=False, norm=None, stem=False, stopwords=True)
